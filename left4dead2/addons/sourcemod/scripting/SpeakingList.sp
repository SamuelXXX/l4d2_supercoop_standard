		...
		for (int i = 1; i <= MaxClients; i++)
		{
			if (ClientSpeakingList[i] > 0)
			{
				if (GetConVarInt(va_teamfilter_speaklist))
				{
					...
					{
						PrintCenterText(i, "Players Speaking:%s", SpeakingPlayers[team]);
					}
				}
				else
					PrintCenterText(i, "Players Speaking:%s%s%s", SpeakingPlayers[0], SpeakingPlayers[1], SpeakingPlayers[2]);
			}
		}
	}
}