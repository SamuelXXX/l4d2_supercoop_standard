	...
	if(action == MenuAction_Select)	{
		...
		if (StrEqual(menuInfo, MMC_ITEM_IDONTCARE_TEXT, false)) {
			...
			if (StrEqual(menuInfo, MMC_ITEM_ALLMAPS_TEXT, false)) {
				...
				if (IsVoteInProgress()) {
					ReplyToCommand(client, "\x03[提示]\x04 %t", "Vote in Progress");
					return 0;
				}
...
public int ChampVoteHandler(Menu menu, MenuAction action, int param1, int param2) {
	if (action == MenuAction_Display) {
		...
		PrintToChatAll("\x03[提示]\x04 %t", "No Votes Cast");
		...
		if (percent < limit) {
			...
			PrintToChatAll("\x03[提示]\x04 %t  \x01[%d,%d,%d]", "Vote Failed", RoundToNearest(100.0*limit), RoundToNearest(100.0*percent), playerCount, yesVotes, noVotes, abstention);
		} else {
			PrintToChatAll("\x03[提示]\x04 %t  \x01[%d,%d,%d]", "Vote Successful", RoundToNearest(100.0*percent), playerCount, yesVotes, noVotes, abstention);
			...
			if (mapIndex < 0) {
				...
				for (int client = 1; client <= MaxClients; client++) {
					if (IsClientInGame(client)) {
						...
						PrintToChat(client,"\x03[提示]\x01 %t\x01", "Mission is now winning the vote", colorizedName);
					}
				}			
			} else {
				...
				for (int client = 1; client <= MaxClients; client++) {
					if (IsClientInGame(client)) {
						LMM_GetMapLocalizedName(g_iGameMode, missionIndex, mapIndex, localizedName, sizeof(localizedName), client);
						Format(colorizedName, sizeof(colorizedName), "\x04%s\x01", localizedName);
						PrintToChat(client,"\x03[提示]\x01 %t", "Map is now winning the vote", colorizedName);
					}
				}
			}
			
			Format(colorizedName, sizeof(colorizedName), "\x04%s\x01", mapName);
			PrintToChatAll("\x03[提示]\x01 %t", "Changing map", colorizedName);
...
public void OnPluginStart() {
	...
	g_hCVar_NextMapMenuExcludes = CreateConVar("acs_next_map_menu_excludes", "0", "Excludes certain map(s) from the map changing menu [0 = Nothing(Default), 1 = Current map ]");
...
public Action Command_ChangeMapVote(int iClient, int args) {
	...
	if(!OnFirstMap())
	{
		ReplyToCommand(iClient,"选图投票仅在第一关可用！");
...
public Action Timer_WelcomeMessage(Handle timer, any param) {
	PrintToChatAll("\x03[提示]\x01 %t", "Change map advertise", "\x04!chmap\x01");
}
...
public void CVarChange_Voting(Handle hCVar, const char[] strOldValue, const char[] strNewValue) {
	...
	if (StringToInt(strNewValue) == 1) {
		PrintToServer("[提示] ConVar changed: Voting System ENABLED");
		PrintToChatAll("[提示] ConVar changed: Voting System ENABLED");
	} else {
		PrintToServer("[提示] ConVar changed: Voting System DISABLED");
		PrintToChatAll("[提示] ConVar changed: Voting System DISABLED");
	}
}
...
public void CVarChange_NewVoteWinnerSound(Handle hCVar, const char[] strOldValue, const char[] strNewValue) {
	...
	//If the value was changed, then set it and display a message to the server and players
	if (StringToInt(strNewValue) == 1) {
		PrintToServer("[提示] ConVar changed: New vote winner sound ENABLED");
		PrintToChatAll("[提示] ConVar changed: New vote winner sound ENABLED");
	} else {
		PrintToServer("[提示] ConVar changed: New vote winner sound DISABLED");
		PrintToChatAll("[提示] ConVar changed: New vote winner sound DISABLED");
	}
}
...
public void CVarChange_VotingAdMode(Handle hCVar, const char[] strOldValue, const char[] strNewValue) {
	...
	switch(StringToInt(strNewValue)) {
		case 0:	{
			PrintToServer("[提示] ConVar changed: Voting display mode: DISABLED");
			PrintToChatAll("[提示] ConVar changed: Voting display mode: DISABLED");
		}
		case 1:	{
			PrintToServer("[提示] ConVar changed: Voting display mode: HINT TEXT");
			PrintToChatAll("[提示] ConVar changed: Voting display mode: HINT TEXT");
		}
		case 2:	{
			PrintToServer("[提示] ConVar changed: Voting display mode: CHAT TEXT");
			PrintToChatAll("[提示] ConVar changed: Voting display mode: CHAT TEXT");
		}
		case 3:	{
			PrintToServer("[提示] ConVar changed: Voting display mode: OPEN VOTE MENU");
...
public void CVarChange_VotingAdDelayTime(Handle hCVar, const char[] strOldValue, const char[] strNewValue) {
	...
	if (fDelayTime > 0.1)
	{
		PrintToServer("[提示] ConVar changed: Voting advertisement delay time changed to %f", fDelayTime);
		PrintToChatAll("[提示] ConVar changed: Voting advertisement delay time changed to %f", fDelayTime);
	}
	else
	{
		PrintToServer("[提示] ConVar changed: Voting advertisement delay time changed to 0.1");
...
public void CVarChange_NewMapAdMode(Handle hCVar, const char[] strOldValue, const char[] strNewValue) {
	...
	switch(StringToInt(strNewValue)) {
		case 0:	{
			PrintToServer("[提示] ConVar changed: Next map advertisement display mode: DISABLED");
			PrintToChatAll("[提示] ConVar changed: Next map advertisement display mode: DISABLED");
		}
		case 1:	{
			PrintToServer("[提示] ConVar changed: Next map advertisement display mode: HINT TEXT");
			PrintToChatAll("[提示] ConVar changed: Next map advertisement display mode: HINT TEXT");
		}
		case 2:	{
			PrintToServer("[提示] ConVar changed: Next map advertisement display mode: CHAT TEXT");
...
public void CVarChange_NewMapAdInterval(Handle hCVar, const char[] strOldValue, const char[] strNewValue) {
	...
	//If the value was changed, then set it and display a message to the server and players
	if (fDelayTime > 60.0) {
		PrintToServer("[提示] ConVar changed: Next map advertisement interval changed to %f", fDelayTime);
		PrintToChatAll("[提示] ConVar changed: Next map advertisement interval changed to %f", fDelayTime);
	} else {
		PrintToServer("[提示] ConVar changed: Next map advertisement interval changed to 60.0");
...
public void CVarChange_MaxFinaleFailures(Handle hCVar, const char[] strOldValue, const char[] strNewValue) {
	...
	//If the value was changed, then set it and display a message to the server and players
	if (iMaxFailures > 0) {
		PrintToServer("[提示] ConVar changed: Max Coop finale failures changed to %f", iMaxFailures);
		PrintToChatAll("[提示] ConVar changed: Max Coop finale failures changed to %f", iMaxFailures);
	} else {
		PrintToServer("[提示] ConVar changed: Max Coop finale failures changed to 0");
...
public Action Event_RoundEnd(Handle hEvent, const char[] strName, bool bDontBroadcast) {
	// PrintToChatAll("\x03[提示]\x04 Event_RoundEnd");
...
public Action Event_FinaleWin(Handle hEvent, const char[] strName, bool bDontBroadcast) {
	// PrintToChatAll("\x03[提示]\x04 Event_FinaleWin");
		...
		if(StrEqual(strCurrentMap, mapName, false)) {
			for (int client = 1; client <= MaxClients; client++) {
				if (IsClientInGame(client)) {
					...
					PrintToChat(client, "\x03[提示]\x05 %t", "Campaign finished", colorizedname);
				}
			}
				...
				if(IsMapValid(mapName)) {
					for (int client = 1; client <= MaxClients; client++) {
						if (IsClientInGame(client)) {
							...
							Format(colorizedname, sizeof(colorizedname), "\x04%s\x05", localizedName);
							PrintToChat(client, "\x03[提示]\x05 %t", "Switching to the vote winner", colorizedname);
						}
					}
			...
			if(IsMapValid(mapName)) {
				for (int client = 1; client <= MaxClients; client++) {
					if (IsClientInGame(client)) {
						ACS_GetLocalizedMissionName(g_iGameMode, cycleIndex, client, localizedName, sizeof(localizedName));
						Format(colorizedname, sizeof(colorizedname), "\x04%s\x05", localizedName);
						PrintToChat(client, "\x03[提示]\x05 %t", "Switching campaign to", colorizedname);
					}
				}
				...
				if(IsMapValid(mapName)) {
					for (int client = 1; client <= MaxClients; client++) {
						if (IsClientInGame(client)) {
							LMM_GetMapLocalizedName(g_iGameMode, missionIndex, mapIndex, localizedName, sizeof(localizedName), client);
							Format(colorizedname, sizeof(colorizedname), "\x04%s\x05", localizedName);
							PrintToChat(client, "\x03[提示]\x05 %t", "Switching map to", colorizedname);
						}
					}	
...
void DisplayNextMapTo_Scavenge(int client, bool replyCMD) {
	...
	if(g_iWinningMapVotes > 0) {
		...
		if (replyCMD) {
			ReplyToCommand(client, "\x03[提示]\x05 %t",
			...
			PrintToChat(client, "\x03[提示]\x05 %t",
				g_iWinningMapIndices_Len == 1 ?
					"The next map is currently" :
					"The next map will be one of",
				strbuf);
		...
		if (replyCMD) {
			Format(colorizedName, sizeof(colorizedName), "\x04%s\x05", localizedName);
			ReplyToCommand(client, "\x03[提示]\x05 %t", "The next map is currently", colorizedName);
		} else if (g_hCVar_NextMapAdMode.IntValue == DISPLAY_MODE_HINT) {
			PrintHintText(client, "%t", "The next map is currently", localizedName);
		} else if (g_hCVar_NextMapAdMode.IntValue == DISPLAY_MODE_CHAT) {
			Format(colorizedName, sizeof(colorizedName), "\x04%s\x05", localizedName);
			PrintToChat(client, "\x03[提示]\x05 %t", "The next map is currently", colorizedName);
		}
	}
}
...
void DisplayNextMapTo(int client, bool replyCMD) {
	...
	if (g_iWinningMapVotes > 0) {
		...
		if (replyCMD) {
			Format(colorizedName, sizeof(colorizedName), "\x04%s\x05", localizedName);
			ReplyToCommand(client, "\x03[提示]\x05 %t", "The next campaign is currently", colorizedName);
		} else if (g_hCVar_NextMapAdMode.IntValue == DISPLAY_MODE_HINT) {
			PrintHintText(client, "%t", "The next campaign is currently", localizedName);
		} else if (g_hCVar_NextMapAdMode.IntValue == DISPLAY_MODE_CHAT) {
			Format(colorizedName, sizeof(colorizedName), "\x04%s\x05", localizedName);
			PrintToChat(client, "\x03[提示]\x05 %t", "The next campaign is currently", colorizedName);
		}
	}
}
...
public Action MapVote(int iClient, int args) {
	if(!g_hCVar_VotingEnabled.BoolValue) {
		ReplyToCommand(iClient, "\x03[提示]\x01 %t", "Voting is disable");
		return;
	}
	
	if(!OnFinaleOrScavengeMap()) {
		PrintToChat(iClient, "\x03[提示]\x01 %t", "Voting is not available");
		return;
	}
...
public Action DisplayCurrentVotes(int iClient, int args) {
	...
	if(hasVoted()) {
		...
		ReplyToCommand(iClient, "\x03[提示]\x01 %t", "No one has voted yet");	
	}
	...
	for(int iClient = 1;iClient <= MaxClients; iClient++) {
		if(
			...
			switch(g_hCVar_VotingAdMode.IntValue) {
				...
				case DISPLAY_MODE_CHAT: PrintToChat(iClient, "\x03[提示]\x05 %t", "Map vote advertise", "\x04!mapvote\x05", "\x04!mapvotes\x05");
			}
