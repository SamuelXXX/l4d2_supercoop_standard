...
public Plugin:myinfo = 
{
    ...
    description = "Disables friendly fire between infected bots.",
    version = "1.3"
}

new Handle:cvar_ffblock;
new Handle:cvar_allow_tank_ff;
new Handle:cvar_block_tank_ff;
...
public OnPluginStart()
{
    ...
    cvar_block_tank_ff=CreateConVar("l4d2_infected_ff_block_tank", "1", "Disable FF towards tankes");
...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3])
{
    if(GetConVarBool(cvar_ffblock) && IsClientAndInGame(victim) && IsClientAndInGame(attacker) && GetClientTeam(attacker) == 3)
    {
        ...
        if (StrEqual(victimClass, "tank")) 
            bIsTank = true;

        if (!GetConVarBool(cvar_allow_tank_ff) || GetEntProp(attacker, Prop_Send, "m_zombieClass") != 8)
        {
            if (!bIsTank && GetClientTeam(victim) == 3)
            {
                return Plugin_Handled;
            }
            else if (bIsTank && GetConVarBool(cvar_block_tank_ff))
            {
                damage = 0.0;
