...
void ShowMenu(int client)
{
	...
	for (int i = 1; i <= MaxClients; i++) 
	{
		...
		if (AvailableBots < 8) // if a one page menu
		{
			...
			{
				Format(m_iHealth, sizeof(m_iHealth), "倒地-血量：%d - ", GetEntData(i, FindDataMapInfo(i, "m_iHealth"), 4));
			}
			else if(GetEntProp(i, Prop_Send, "m_currentReviveCount") == FindConVar("survivor_max_incapacitated_count").IntValue)
			{
				Format(m_iHealth, sizeof(m_iHealth), "黑白-血量：%d - ", GetEntData(i, FindDataMapInfo(i, "m_iHealth"), 4));
			}
			else
			{
				Format(m_iHealth, sizeof(m_iHealth), "血量：%d - ", GetClientRealHealth(i));
			}
		}
